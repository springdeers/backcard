...
int protobuffer_send(p2pclient_t p2p,int msgid,void* pdata)
{
	...
	switch(msgid){
	case eProto_initsession: //向printsvr发送连接认证
		{
			...
			response_pkt_p pkt;
			if (initsess == NULL){
				count += sprintf_s(buffer + count, sizeof(buffer) - count, "$initsess,%s,%s,{\"role\":\"%s\",\"seqno\":\"%d\"}", "backcard", "printsvr", "backcard", g_seqno2printsvr);
			}else{
				count += sprintf_s(buffer + count, sizeof(buffer) - count, "$initsess,%s,%s,{\"role\":\"%s\",\"seqno\":\"%d\"}", initsess->from, initsess->to, "backcard", g_seqno2printsvr);
			}
	...
	case eProto_auth:
	{
		...
		count += sprintf_s(buffer + count, sizeof(buffer) - count, "$auth,%s,%s,{\"pwd\":\"%s\",\"name\":\"%s\",\"seqno\":\"%d\"}", "backcard", "printsvr","123456","front1", g_seqno2printsvr);
...
int protobuffer_send_p2p(p2p_t p2p, int msgid, void* pdata)
{
	...
	case eProto_initack:
	{
		...
		response_pkt_p pkt;
		if (initack == NULL){
			count += sprintf_s(buffer + count, sizeof(buffer) - count, "$initack,{\"from\":\"%s\",\"to\":\"%s\"}", "backcard", "moniclient");//obsolete..
		}
		else{
	...
	case eProto_authack:
	{
		...
		count += sprintf_s(buffer + count, sizeof(buffer) - count, "$authack,%s,%s,{\"rslt\":\"%s\",\"error\":\"%s\",\"seqno\" : \"%d\"}", "backcard",auth->peername, auth->ok ? "ok" : "fail", auth->ok ? "":auth->err,auth->seqno);
		count += sprintf_s(buffer + count, sizeof(buffer) - count, "*%02X\r\n", checksum(buffer, count));
		endpos = count;
		buffer[endpos + 1] = '\0';
		pkt = response_pkt_new(client()->pktpool, endpos + 1);
