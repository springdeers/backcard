...
static int search_frame(printsvr_exp_t explain, unsigned char*p, int len, int* searchpos);
static int  explain(printsvr_exp_t exp, uchar*frame, int len);
...
{
	printsvr_exp_t rtn = NULL;

	while ((rtn = (printsvr_exp_t)calloc(1, sizeof(printsvr_exp_st))) == NULL);
...
void    printsvr_exp_free(printsvr_exp_t exp)
{
	if (exp) free(exp);
}
