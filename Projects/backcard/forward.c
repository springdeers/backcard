...
void forward_to_app(char *head, char* jstr)
{
	...
	// 如果网络类型是北斗，则直接转发给moniclient
	if (strcmp("bd", net) == 0)
	{
		char *moniclient = "moniclient01";
...
void forward_to_printsvr(char *head, char* jstr)
{
	...
	if (client()->p2p_printsvr != NULL)
	{
		...
		log_write(client()->log, LOG_NOTICE, "send %s ack to printsvr:%s.", head, buffer);
	}
}
...
void forward_to_monitor(char *head, char *jstr)
{
	...
	response_pkt_p pkt = NULL;
	unsigned char ucheck;
	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$%s,%s,%s,%s", head, "backcard", "monitor", jstr);
	ucheck = checksum((unsigned char*)buffer, cnt);
	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
	buffer[cnt] = '\0';
	p2p = (p2p_t)destsess;
...
void forward_log_x_to_monitor(char *head, char *jstr)
{
	...
	count += sprintf_s(buffer + count, sizeof(buffer) - count, "$log,%s,%s,{\"taskid\":\"%s\",\"executor\":\"%s\",\"net\":\"%s\",\"data\":{\"time\":\"%s\",\"type\":\"%s\"},\"loc\":{\"lon\":\"%s\",\"lat\":\"%s\",\"hgt\":\"%s\"}}", "backcard", "monitor", taskid, executor, net, strtime, head, lon, lat, hgt);
	count += sprintf_s(buffer + count, sizeof(buffer) - count, "*%02X\r\n", checksum((unsigned char *)buffer, count));
	endpos = count;
	buffer[endpos] = '\0';
...
void forward_log_dyncode_to_monitor(char *jstr)
{
	...
	count += sprintf_s(buffer + count, sizeof(buffer) - count, "$log,%s,%s,{\"taskid\":\"%s\",\"executor\":\"%s\",\"net\":\"%s\",\"data\":{\"time\":\"%s\",\"type\":\"%s\"},\"loc\":{\"lon\":\"%s\",\"lat\":\"%s\",\"hgt\":\"%s\"}}", "backcard", "monitor", taskid, executor, net, strtime, "dyncode", lon, lat, hgt);
	count += sprintf_s(buffer + count, sizeof(buffer) - count, "*%02X\r\n", checksum((unsigned char *)buffer, count));
	endpos = count;
	buffer[endpos] = '\0';
...
//	response_pkt_p pkt = NULL;
//	unsigned char ucheck;
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$dyncode,%s,%s,%s", "backcard", destsess->sname, jstr);
//	ucheck = checksum((unsigned char*)buffer, cnt);
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
//	buffer[cnt] = '\0';
//	p2p = (p2p_t)destsess;
...
void forward_warn_to_monitor(char * jstr)
{
	...
	response_pkt_p pkt = NULL;
	unsigned char ucheck;
	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$warn,%s,%s,%s", "backcard", "monitor", jstr);
	ucheck = checksum((unsigned char*)buffer, cnt);
	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
	buffer[cnt] = '\0';
	p2p = (p2p_t)destsess;
...
void forward_warn_to_app(char *jstr)
{
	...
	if (strcmp("inet", net) == 0)
	{
		...
		response_pkt_p pkt = NULL;
		unsigned char ucheck;
		cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$warn,%s,%s,%s", "backcard", destsess->sname, jstr);
		ucheck = checksum((unsigned char*)buffer, cnt);
		cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
		buffer[cnt] = '\0';
		p2p = (p2p_t)destsess;
...
//	response_pkt_p pkt;
//
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "$reqcode,%s,%s,%s", "backcard", "printsvr", jstr);
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "*%02X\r\n", checksum((unsigned char *)buffer, count));
//	endpos = count;
//	buffer[endpos] = '\0';
//
...
//		p2pclient_write((p2pclient_t)(client()->p2p_printsvr), pkt);
//
//		log_write(client()->log, LOG_NOTICE, "send reqcode to printsvr:%s.", buffer);
//	}
//}
//
...
//	response_pkt_p pkt = NULL;
//	unsigned char ucheck;
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$taskpau,%s,%s,%s", "backcard", "monitor", jstr);
//	ucheck = checksum((unsigned char*)buffer, cnt);
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
//	buffer[cnt] = '\0';
//	p2p = (p2p_t)destsess;
...
//	response_pkt_p pkt;
//
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "$log,%s,%s,{\"taskid\":\"%s\",\"executor\":\"%s\",\"net\":\"%s\",\"data\":{\"time\":\"%s\",\"type\":\"%s\"},\"loc\":{\"lon\":\"%s\",\"lat\":\"%s\",\"hgt\":\"%s\"}}", "backcard", "monitor", taskid, executor, net, strtime, "taskpause", lon, lat, hgt);
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "*%02X\r\n", checksum((unsigned char *)buffer, count));
//	endpos = count;
//	buffer[endpos] = '\0';
//
...
//	response_pkt_p pkt = NULL;
//	unsigned char ucheck;
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$taskcon,%s,%s,%s", "backcard", "monitor", jstr);
//	ucheck = checksum((unsigned char*)buffer, cnt);
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
//	buffer[cnt] = '\0';
//	p2p = (p2p_t)destsess;
...
//	response_pkt_p pkt;
//
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "$log,%s,%s,{\"taskid\":\"%s\",\"executor\":\"%s\",\"net\":\"%s\",\"data\":{\"time\":\"%s\",\"type\":\"%s\"},\"loc\":{\"lon\":\"%s\",\"lat\":\"%s\",\"hgt\":\"%s\"}}", "backcard", "monitor", taskid, executor, net, strtime, "taskcontinue", lon, lat, hgt);
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "*%02X\r\n", checksum((unsigned char *)buffer, count));
//	endpos = count;
//	buffer[endpos] = '\0';
//
...
//	response_pkt_p pkt = NULL;
//	unsigned char ucheck;
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$taskdone,%s,%s,%s", "backcard", "monitor", jstr);
//	ucheck = checksum((unsigned char*)buffer, cnt);
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
//	buffer[cnt] = '\0';
//	p2p = (p2p_t)destsess;
...
//	response_pkt_p pkt;
//
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "$log,%s,%s,{\"taskid\":\"%s\",\"executor\":\"%s\",\"net\":\"%s\",\"data\":{\"time\":\"%s\",\"type\":\"%s\"},\"loc\":{\"lon\":\"%s\",\"lat\":\"%s\",\"hgt\":\"%s\"}}", "backcard", "monitor", taskid, executor, net, strtime, "taskdone", lon, lat, hgt);
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "*%02X\r\n", checksum((unsigned char *)buffer, count));
//	endpos = count;
//	buffer[endpos] = '\0';
//
...
//	response_pkt_p pkt = NULL;
//	unsigned char ucheck;
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$taskpau,%s,%s,%s", "backcard", destsess->sname, jstr);
//	ucheck = checksum((unsigned char*)buffer, cnt);
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
//	p2p = (p2p_t)destsess;
//	pkt = response_pkt_new(client()->pktpool, cnt);
...
//	response_pkt_p pkt;
//
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "$taskdone,%s,%s,%s", "backcard", "printsvr", jstr);
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "*%02X\r\n", checksum((unsigned char *)buffer, count));
//	endpos = count;
//	buffer[endpos + 1] = '\0';
//
...
//		p2pclient_write((p2pclient_t)(client()->p2p_printsvr), pkt);
//
//		log_write(client()->log, LOG_NOTICE, "send taskdone ack to printsvr:%s.", buffer);
//	}
//}
//
...
//	response_pkt_p pkt = NULL;
//	unsigned char ucheck;
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$taskdone,%s,%s,%s", "backcard", destsess->sname, jstr);
//	ucheck = checksum((unsigned char*)buffer, cnt);
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
//	p2p = (p2p_t)destsess;
//	pkt = response_pkt_new(client()->pktpool, cnt);
...
//	response_pkt_p pkt = NULL;
//	unsigned char ucheck;
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$sta,%s,%s,%s", "backcard", "monitor", jstr);
//	ucheck = checksum((unsigned char*)buffer, cnt);
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
//	buffer[cnt] = '\0';
//	p2p = (p2p_t)destsess;
...
//	response_pkt_p pkt = NULL;
//	unsigned char ucheck;
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$log,%s,%s,%s", "backcard", "monitor", jstr);
//	ucheck = checksum((unsigned char*)buffer, cnt);
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
//	buffer[cnt] = '\0';
//	p2p = (p2p_t)destsess;
...
//	response_pkt_p pkt;
//
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "$pos,%s,%s,%s", "backcard", "printsvr", jstr);
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "*%02X\r\n", checksum((unsigned char *)buffer, count));
//	endpos = count;
//	buffer[endpos + 1] = '\0';
//
//	p2pclient_t conn = client()->p2p_printsvr;
...
//		else
//			log_write(client()->log, LOG_NOTICE, "send pos to printsvr:%s.", buffer);
//	}
//}
//
...
//	response_pkt_p pkt = NULL;
//	unsigned char ucheck;
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$pos,%s,%s,%s", "backcard", "monitor", jstr);
//	ucheck = checksum((unsigned char*)buffer, cnt);
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
//	buffer[cnt] = '\0';
//	p2p = (p2p_t)destsess;
...
//	response_pkt_p pkt;
//
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "$taskpau,%s,%s,%s", "backcard", "printsvr", jstr);
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "*%02X\r\n", checksum((unsigned char *)buffer, count));
//	endpos = count;
//	buffer[endpos + 1] = '\0';
//
...
//		p2pclient_write((p2pclient_t)(client()->p2p_printsvr), pkt);
//
//		log_write(client()->log, LOG_NOTICE, "send taskpau ack to printsvr:%s.", buffer);
//	}
//}
//
...
//	response_pkt_p pkt;
//
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "$taskcon,%s,%s,%s", "backcard", "printsvr", jstr);
//	count += sprintf_s(buffer + count, sizeof(buffer) - count, "*%02X\r\n", checksum((unsigned char *)buffer, count));
//	endpos = count;
//	buffer[endpos + 1] = '\0';
//
...
//		p2pclient_write((p2pclient_t)(client()->p2p_printsvr), pkt);
//
//		log_write(client()->log, LOG_NOTICE, "send taskcon ack to printsvr:%s.", buffer);
//	}
//}
//
...
//	response_pkt_p pkt = NULL;
//	unsigned char ucheck;
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "$taskcon,%s,%s,%s", "backcard", destsess->sname, jstr);
//	ucheck = checksum((unsigned char*)buffer, cnt);
//	cnt += sprintf_s(buffer + cnt, sizeof(buffer) - cnt, "*%02X\r\n", ucheck);
//	p2p = (p2p_t)destsess;
//	pkt = response_pkt_new(client()->pktpool, cnt);
